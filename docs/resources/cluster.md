...
* `kubeconfig` - (Optional) Kubeconfig set after the the cluster is created.
* `kubeconfig_path` (Optional) Kubeconfig path set after the the cluster is created.
* `client_certificate` (Optional) Client certificate for authenticating to cluster.
* `client_key` (Optional) Client key for authenticating to cluster.
* `cluster_ca_certificate` (Optional) Client verifies the server certificate with this CA cert.
* `endpoint` (Optional) Kubernetes API server endpoint.
